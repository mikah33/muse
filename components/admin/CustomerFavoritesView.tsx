...
export default function CustomerFavoritesView({ customerId }: CustomerFavoritesViewProps) {
    ...
    <div className="bg-white rounded-lg shadow p-8">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-semibold">Customer Favorites</h2>
        </div>
      ...
      {favorites.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 text-sm mt-2">
            This customer has not favorited any photos
          </p>
        </div>
      ) : (
        /* Grid */
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {favorites.map((favorite) => (
            <div
              key={favorite.id}
              className="group relative aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-black transition-all"
              onClick={() => setSelectedPhoto(favorite.photo.photo_url)}
            >
              <Image
                src={favorite.photo.thumbnail_url}
                alt={favorite.photo.title || 'Favorited photo'}
                fill
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {favorite.photo.gallery && (
                    <p className="text-white/80 text-xs truncate">
                      {favorite.photo.gallery.gallery_name}
                    </p>
                  )}
